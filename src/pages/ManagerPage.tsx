  ...
  const handleSave = async () => {
    ...
    try {
      const { error: expErr } = await sb.from("owner_expenses").insert({
        owner_id: ownerId, amount: total, description, expense_date: today,
      });
      ...
      await sb.from("wallet_transactions").insert({
        profile_id: profile.id, amount: total, type: "cashier_expense", note: expenseNote,
      });
      ...
      <div className="rounded-3xl p-4 space-y-3 relative overflow-hidden"
        ...
        <div className="grid grid-cols-3 gap-2 relative">
          {[
            ...
            { label: "Today's\nExpense",  value: barIsOpen ? `$${fmt(todayExpenses)}` : "—" },
            { label: "Total\nExpense",    value: totalAllTime > 0 ? `$${fmt(totalAllTime)}` : "$0.00" },
          ].map((c) => (
            <div key={c.label} className="rounded-2xl p-2.5 flex flex-col gap-0.5 text-center" style={{ background: "oklch(0.18 0.02 60)" }}>
              <div className="text-[9px] font-semibold leading-tight whitespace-pre-line" style={{ color: "rgba(255,255,255,0.5)" }}>{c.label}</div>
              ...
              {lines.map((line, i) => (
                <div key={i} className="space-y-1.5">
                  ...
                  <div className="flex gap-2 items-center">
                    <input value={line.amount} onChange={(e) => updateLine(i, "amount", e.target.value)}
              ...
              <div key={mk} className="rounded-2xl border border-border overflow-hidden" style={{ background: "var(--gradient-card)" }}>
                <button type="button" onClick={() => setOpenMonth(isOpen ? null : mk)}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-muted-foreground" style={{ display: "inline-block", transform: isOpen ? "rotate(90deg)" : "rotate(0deg)" }}>▶</span>
                  </div>
                </button>
                {isOpen && (
                          ...
                          {isEditing ? (
                            <div className="space-y-2">
                              ...
                              <div className="grid grid-cols-2 gap-2 pt-1">
                                <button onClick={cancelEdit} className="h-9 rounded-xl font-black text-xs border border-border transition active:scale-95">Cancel</button>
                                ...
                                {canEdit && (
                                  <div className="flex gap-1 mt-0.5">
                                    <button onClick={() => startEdit(e)}
                                      ...
                                      style={{ background: "rgba(255,255,255,0.08)" }} title="Edit">
                                      <Pencil className="h-3 w-3 text-muted-foreground" />
                                    </button>
                                    <button onClick={() => setDeleteConfirmId(e.id)}
                                      className="h-7 w-7 rounded-lg flex items-center justify-center transition active:scale-90"
                                      style={{ background: "rgba(239,68,68,0.12)" }} title="Delete">
