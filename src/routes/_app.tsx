...
function AppLayout() {
  ...
  useEffect(() => {
    ...
    const barOwnerId = effectiveOwnerId(
      isManagerProfile ? (profile.parent_id ?? profile.id) : profile.id,
    );
    ...
    const ch = supabase
      ...
      .on(
        "postgres_changes",
        { event: "UPDATE", schema: "public", table: "profiles", filter: `id=eq.${barOwnerId}` },
        (payload) => {
  ...
  const confirmOpenBar = async () => {
    ...
    // 1a. Clear all cashier wallet records for new bar session
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  useEffect(() => {
    ...
    (supabase as any)
      ...
      .single()
      .then(({ data }: any) => {
        setManagerHasMachinesNav(
          !!data?.machines_addon_active ||
            data?.plan_type === "premium" ||
            data?.plan_type === "chain",
        );
      });
            ...
            <div className="px-6 pb-6 pt-4 space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  ...
                  disabled={
                    (!isMachinesAccount && !openBarFloat) || (hasMachines && !openMachineFloat)
                  }
                  className="flex-1 h-12 rounded-2xl font-black text-sm transition active:scale-95 disabled:opacity-50"
                  style={{
