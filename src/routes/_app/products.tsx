...
const STOCK_BTNS = [{ qty: 30 }, { qty: 24 }, { qty: 12 }, { qty: 10 }, { qty: 6 }, { qty: 1 }];

function StockNumpad({
  productId,
  productName,
  ownerId,
  currentQty,
  ...
  const untap = (i: number) =>
    setCounts((c) => c.map((v, j) => (j === i ? Math.max(0, v - 1) : v)));
  ...
  const canUndo =
    stockQtyUndo !== null && stockQtyUndoSaved !== null && currentQty === stockQtyUndoSaved;
      ...
      >
        <div className="relative flex items-center justify-center px-5 pt-5 pb-3">
          <button
            ...
            className="absolute left-5 flex items-center gap-1 text-sm font-bold text-muted-foreground hover:text-foreground transition"
          >
    ...
    return (
      <div className="flex flex-col items-center justify-center text-center text-muted-foreground gap-2 py-12">
        ...
        <p className="text-xs opacity-60">
          Ask your admin to import some from the Admin → Import tab.
        </p>
      </div>
    );
  }
      ...
      {visible.map((t) => {
        ...
        return (
          <button
            key={t.url}
            onClick={() =>
              isMulti ? onToggle!(t.url, t.label, category) : onSelect(t.url, t.label, category)
            }
          ...
          >
            ...
            <div className="absolute inset-x-0 bottom-0 p-2 bg-gradient-to-t from-black/85 to-transparent pointer-events-none">
              <div className="text-white text-xs font-bold leading-tight line-clamp-2">
                {t.label}
              </div>
            </div>
          </button>
...
function BulkEditModal({
  ...
  const [unitsPerItems, setUnitsPerItems] = useState<Record<string, string>>(() =>
    Object.fromEntries(
      items.map((p) => [p.id, p.units_per_item > 0 ? String(p.units_per_item) : ""]),
    ),
  );
  ...
  const [activeNumpad, setActiveNumpad] = useState<{
    id: string;
    field: "cp" | "sp" | "qty" | "units" | "vp" | "vu" | "rp" | "sq" | "spp";
  } | null>(null);
  ...
  const [cigSpecialData, setCigSpecialData] = useState<
  ...
  >(() => {
    ...
    items.forEach((p) => {
      if (p.category === "cigarettes") {
        ...
        const specials = (p.bottle_variations ?? []).filter(
          (v) => v.key === "special" || v.key.startsWith("special_"),
        );
        init[p.id] =
          specials.length > 0
  ...
  const handleNumpad = (k: string) => {
    ...
    const isDecimal =
      field === "cp" || field === "sp" || field === "vp" || field === "rp" || field === "spp";
    ...
    if (field === "rp") {
      ...
      if (k === "⌫") {
        ...
        const di = cur.indexOf(".");
        if (di !== -1 && cur.length - di > 2) return;
        next = cur === "0" ? k : cur + k;
      }
    ...
    if (k === ".") {
      if (isDecimal && !current.includes(".")) setter(current + ".");
      return;
    }
  ...
  const priceOnlyChanges = items.filter((p) => {
    ...
    const retailChanged =
      ...
      !isNaN(newRetail) &&
      newRetail !== Number(existRetail?.price ?? 0);
    ...
    const specialChanged =
      ...
      (() => {
        ...
        return rows.some((row, i) => {
          ...
          return (
            (!isNaN(sq) && sq !== Number(ex?.units_consumed ?? 0)) ||
  ...
  const save = async () => {
    ...
    if (totalCost > 0) {
      const { data: expData, error: expErr } = await supabase
        ...
        .single();
      if (expErr) {
        toast.error("Could not create expense record: " + expErr.message);
        setBusy(false);
        return;
      }
    ...
    for (const p of updates) {
      ...
      const batchTotalCost = parseFloat(costPrices[p.id] ?? "");
      const newSp = parseFloat(sellPrices[p.id] ?? "");
      const newUnits = parseInt(unitsPerItems[p.id] ?? "", 10);
      const spChanged = !isNaN(newSp) && newSp !== Number(p.price ?? 0);
      const unitsChanged = !isNaN(newUnits) && newUnits !== Number(p.units_per_item ?? 0);

      // Derive per-item cost from total batch cost
      const newCpPerItem =
        addQty > 0 && !isNaN(batchTotalCost) && batchTotalCost > 0
          ? batchTotalCost / addQty
      ...
      const finalCp = totalQty > 0 ? totalValue / totalQty : 0;
      ...
      const { error } = await supabase
        ...
        .eq("id", p.id);
      if (error) {
        toast.error(`Failed to update ${p.name}: ${error.message}`);
      } else {
        patches.push({
          id: p.id,
          stock_qty: newTotal,
    ...
    for (const p of items.filter((p) => !updates.includes(p))) {
      ...
      if (p.category === "cigarettes") {
        ...
        const specialChanged =
          rows.length !== existSpecials.length ||
          rows.some((row, i) => {
            const sq = parseInt(row.qty, 10);
            const sp = parseFloat(row.price);
            return (
              (!isNaN(sq) && sq !== Number(existSpecials[i]?.units_consumed ?? 0)) ||
              (!isNaN(sp) && sp !== Number(existSpecials[i]?.price ?? 0))
            );
          });
        if (retailChanged || specialChanged) {
          cigVarsChanged = true;
          const retailEntry: BottleVariation | null =
            !isNaN(newRetail) && newRetail > 0
              ? { key: "retail", label: "Retail", units_consumed: 1, price: newRetail }
              : (existRetail ?? null);
          cigVarsMerged = [...(retailEntry ? [retailEntry] : []), ...specialEntries];
        }
      }
      if (
        !cpChanged &&
        ...
        if (retailChanged || specialChanged) {
          const retailEntry: BottleVariation | null =
            !isNaN(newRetail) && newRetail > 0
              ? { key: "retail", label: "Retail", units_consumed: 1, price: newRetail }
              : (existRetail ?? null);
          cigMerged = [...(retailEntry ? [retailEntry] : []), ...specialEntries];
        }
      }
  ...
  // shared input style
  const numInputCls =
    "w-full h-8 rounded-lg border text-right pr-2 text-xs font-black bg-muted/50 outline-none focus:ring-1 focus:ring-primary transition";
    ...
    <>
      <div className="fixed inset-0 z-[70] flex flex-col bg-background" onClick={onClose}>
        ...
        >
          ...
          >
            ...
            <button
              onClick={onClose}
              className="h-10 px-5 rounded-xl font-black text-sm flex items-center gap-2 bg-muted hover:bg-muted/80 transition active:scale-95"
            >
            ...
            <table className="w-full border-collapse text-sm">
              <thead className="sticky top-0 z-10 bg-background border-b border-border">
                <tr>
                  ...
                  <th className="text-left px-2 py-2 font-black text-xs text-muted-foreground">
                    Name
                  </th>
                  <th className="text-right px-2 py-2 font-black text-xs text-muted-foreground w-[76px] sm:w-[96px]">
                    Cost
                  </th>
                  <th className="text-right px-2 py-2 font-black text-xs text-muted-foreground w-[76px] sm:w-[96px]">
                    Sell
                  </th>
                  <th className="text-right px-2 py-2 font-black text-xs text-muted-foreground w-[56px] leading-tight">
                    Drink/
                    <br />
                    Retail
                  </th>
                  <th className="text-right px-2 py-2 font-black text-xs text-muted-foreground w-[46px] sm:w-[60px]">
                    Qty
                  </th>
                  <th
                    className="text-right pr-4 pl-2 py-2 font-black text-xs w-[76px] sm:w-[96px]"
                        ...
                        <React.Fragment key={p.id}>
                          ...
                          >
                            ...
                            <td className="pl-3 pr-2 py-1.5 w-10 sm:w-14">
                              <div
                                className="h-8 w-8 sm:h-12 sm:w-12 rounded-lg overflow-hidden border border-border shrink-0 flex items-center justify-center text-base sm:text-xl"
                                style={{ background: "var(--gradient-card)" }}
                              >
                                {p.image_url ? (
                                  <img
                            ...
                            <td className="px-2 py-1.5 w-[76px] sm:w-[96px]">
                              <div
                                onClick={() =>
                                  setActiveNumpad(
                                    activeNumpad?.id === p.id && activeNumpad.field === "sp"
                                      ? null
                                      : { id: p.id, field: "sp" },
                                  )
                                }
                            ...
                            <td className="px-2 py-1.5 w-[56px]">
                              {isBottleOrPack ? (
                                <div
                                  onClick={() =>
                                    setActiveNumpad(
                                      activeNumpad?.id === p.id && activeNumpad.field === "units"
                                        ? null
                                        : { id: p.id, field: "units" },
                                    )
                                  }
                                ...
                                <span className="text-xs text-muted-foreground flex justify-end pr-2">
                                  —
                                </span>
                              )}
                            </td>
                            ...
                            <td className="px-2 py-1.5 text-right w-[46px] sm:w-[60px]">
                              <div className="flex items-center justify-end gap-1">
                                <span
                                  className={`font-black text-xs sm:text-sm ${(liveQtys[p.id] ?? 0) === 0 ? "text-red-400" : (liveQtys[p.id] ?? 0) <= 5 ? "text-yellow-400" : "text-green-400"}`}
                                >
                                  {liveQtys[p.id] ?? 0}
                                </span>
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    setRevertItem({
                                      ...p,
                                      stock_qty: liveQtys[p.id] ?? p.stock_qty,
                                    })
                                  }
                            ...
                            <td className="pr-4 pl-2 py-1.5 text-right w-[76px] sm:w-[96px]">
                              <div
                                onClick={() =>
                                  setActiveNumpad(
                                    activeNumpad?.id === p.id && activeNumpad.field === "qty"
                                      ? null
                                      : { id: p.id, field: "qty" },
                                  )
                                }
                          ...
                          {(p.bottle_variations ?? []).map((bv) => {
                            ...
                            const isActive =
                              activeNumpad?.id === varKey && activeNumpad.field === "vp";
                            return (
                              <tr
                                key={varKey}
                              ...
                              >
                                ...
                                <td className="px-2 py-1">
                                  <span className="text-[10px] font-semibold text-muted-foreground pl-3">
                                    ↳ {bv.key === "shot" ? "Drink" : bv.label}
                                  </span>
                                ...
                                <td className="px-2 py-1" colSpan={2}>
                                  <div
                                    onClick={() =>
                                      setActiveNumpad(isActive ? null : { id: varKey, field: "vp" })
                                    }
                          ...
                          {p.category === "liquor" &&
                            (localVars[p.id] ?? []).map((lv, lvIdx) => {
                              const vuKey = `${p.id}__varIdx__${lvIdx}`;
                              const vpKey = `${p.id}__localVar__${lvIdx}`;
                              const isVuActive =
                                activeNumpad?.id === vuKey && activeNumpad.field === "vu";
                              const isVpActive =
                                activeNumpad?.id === vpKey && activeNumpad.field === "vp";
                                ...
                                >
                                  ...
                                  <td className="px-1 py-1">
                                    <div
                                      onClick={() =>
                                        setActiveNumpad(
                                          isVuActive ? null : { id: vuKey, field: "vu" },
                                        )
                                      }
                                      className="h-7 rounded-lg border text-right pr-2 text-xs font-black bg-muted/50 flex items-center justify-end cursor-pointer active:bg-muted/70 transition"
                                      style={{
                                        borderColor: isVuActive
                                          ? "var(--primary)"
                                          : "var(--border)",
                                        color: "var(--muted-foreground)",
                                      }}
                                  ...
                                  <td className="px-1 py-1">
                                    <div
                                      onClick={() =>
                                        setActiveNumpad(
                                          isVpActive ? null : { id: vpKey, field: "vp" },
                                        )
                                      }
                                      className="h-7 rounded-lg border text-right pr-2 text-xs font-black bg-muted/50 flex items-center justify-end cursor-pointer active:bg-muted/70 transition"
                                      style={{
                                        borderColor: isVpActive
                                          ? "var(--primary)"
                                          : "var(--border)",
                                        color: "var(--foreground)",
                                      }}
                          ...
                          {p.category === "liquor" && (
                            <tr className="border-t border-border/10">
                              <td colSpan={8} className="pl-8 py-1.5">
                                ...
                                >
                                  <span
                                    className="h-4 w-4 rounded-full flex items-center justify-center text-black text-[10px] font-black"
                                    style={{ background: "var(--gradient-hero)" }}
                                  >
                                    +
                                  </span>
                          ...
                          {p.category === "cigarettes" &&
                            (() => {
                              ...
                              const isRpActive =
                                activeNumpad?.id === p.id && activeNumpad.field === "rp";
                              ...
                              return (
                                <>
                                  ...
                                  >
                                    ...
                                    <td className="px-2 py-1" colSpan={2}>
                                      <div
                                        onClick={() =>
                                          setActiveNumpad(
                                            isRpActive ? null : { id: p.id, field: "rp" },
                                          )
                                        }
                                        className="h-7 rounded-lg border text-right pr-2 text-xs font-black bg-muted/50 flex items-center justify-end cursor-pointer active:bg-muted/70 transition"
                                        style={{
                                          borderColor: isRpActive
                                            ? "var(--primary)"
                                            : "var(--border)",
                                          color: "var(--foreground)",
                                        }}
                                  ...
                                  {specials.map((sd, specIdx) => {
                                    ...
                                    const isSqActive =
                                      activeNumpad?.id === rowId && activeNumpad.field === "sq";
                                    const isSppActive =
                                      activeNumpad?.id === rowId && activeNumpad.field === "spp";
                                    return (
                                      <tr
                                        key={rowId}
                                        className="border-t border-border/20"
                                      ...
                                      >
                                        ...
                                        <td className="px-2 py-1">
                                          <span
                                            className="text-[10px] font-semibold pl-3"
                                            style={{ color: "var(--primary)" }}
                                          >
                                        ...
                                        <td className="px-2 py-1">
                                          <div
                                            onClick={() =>
                                              setActiveNumpad(
                                                isSqActive ? null : { id: rowId, field: "sq" },
                                              )
                                            }
                                            ...
                                            style={{
                                              borderColor: isSqActive
                                                ? "var(--primary)"
                                                : "var(--border)",
                                            }}
                                            title="Qty in bundle"
                                          >
                                            <span className="pl-2 text-muted-foreground/60 font-semibold shrink-0">
                                              qty:
                                            </span>
                                            <span
                                        ...
                                        <td className="px-2 py-1">
                                          <div
                                            onClick={() =>
                                              setActiveNumpad(
                                                isSppActive ? null : { id: rowId, field: "spp" },
                                              )
                                            }
                                            className="h-7 rounded-lg border text-right pr-2 text-xs font-black bg-muted/50 flex items-center justify-end cursor-pointer active:bg-muted/70 transition"
                                            style={{
                                              borderColor: isSppActive
                                                ? "var(--primary)"
                                                : "var(--border)",
                                              color: "var(--foreground)",
                                            }}
          ...
          {activeNumpad && (
            <div
              className="shrink-0 border-t border-border px-4 pt-3 pb-2"
              style={{ background: "var(--background)" }}
            >
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-black text-muted-foreground uppercase tracking-widest">
                  {activeNumpad.field === "cp"
                    ? t("numpad_total_cost", "Total Cost")
                    : activeNumpad.field === "sp"
                      ? t("numpad_sell", "Sell Price")
                      : activeNumpad.field === "units"
                        ? t("numpad_units", "Units per Item")
                        : activeNumpad.field === "vp"
                          ? t("numpad_var_price", "Variation Price")
                          : activeNumpad.field === "vu"
                            ? t("numpad_drinks_used", "Drinks Used")
                            : t("numpad_add_qty", "Add Qty")}
                </span>
                <button
              ...
              <div className="grid grid-cols-3 gap-1.5">
                {[
                  ...
                  "8",
                  "9",
                  activeNumpad.field !== "qty" && activeNumpad.field !== "vu" ? "." : "",
                  "0",
                  "⌫",
                ].map((k, i) =>
                  k === "" ? (
          ...
          <div className="flex flex-col h-full w-full max-w-2xl">
            ...
            <div className="flex items-center justify-between px-4 pt-5 pb-3 border-b border-border shrink-0">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  {allChanged.length} item{allChanged.length !== 1 ? "s" : ""} will be updated
                </p>
              </div>
              <button
            ...
            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2">
              {allChanged.map((p) => {
                ...
                // Per-item cost from total cost / add qty
                const batchTotal = hasQty ? parseFloat(costPrices[p.id] ?? "") || 0 : 0;
                const newCpPerItem =
                  hasQty && batchTotal > 0 ? batchTotal / addQty : Number(p.cost_price ?? 0);

                // Weighted average preview (matches save logic)
                const oldQty = p.stock_qty ?? 0;
                const oldValue = oldQty * Number(p.cost_price ?? 0);
                const newValue = addQty * newCpPerItem;
                const totalValue = oldValue + newValue;
                const totalQty = oldQty + addQty;
                const previewCp = totalQty > 0 ? totalValue / totalQty : 0;
                const lineTotal = hasQty ? batchTotal : 0;
                const cpChanged = hasQty && previewCp !== Number(p.cost_price ?? 0);
                return (
                  <div
                    key={p.id}
                    ...
                    <div className="flex-1 min-w-0 space-y-1">
                      ...
                      {hasQty && (
                        <div className="text-xs font-bold" style={{ color: "var(--primary)" }}>
                          Stock: {p.stock_qty ?? 0} →{" "}
                          <span className="text-green-400">{(p.stock_qty ?? 0) + addQty}</span>
                          <span className="text-muted-foreground ml-2">
                            (+{addQty} total ${batchTotal.toFixed(2)})
                          </span>
                        </div>
                      )}
                      {cpChanged && (
                        <div className="text-xs text-muted-foreground">
                          ...
                          {hasQty && batchTotal > 0 && (
                            <span className="text-muted-foreground/70 ml-1">
                              (avg of {oldQty} @ ${Number(p.cost_price ?? 0).toFixed(2)} + {addQty}{" "}
                      ...
                      {spChanged && (
                        <div className="text-xs text-muted-foreground">
                          Sell:{" "}
                          <span className="line-through">${Number(p.price ?? 0).toFixed(2)}</span>
                          <span className="text-yellow-400 font-black ml-1">
                            {" "}
                            → ${newSp.toFixed(2)}
                          </span>
              ...
              {totalCost > 0 && (
                <div
                  className="rounded-2xl border border-green-500/30 px-4 py-3 flex items-center justify-between mt-2"
      ...
      {revertItem && (
        <RevertStockModal
          ...
          costPrice={
            parseFloat(costPrices[revertItem.id] ?? "") || Number(revertItem.cost_price ?? 0)
          }
...
export default function ProductsPage() {
  ...
  const [stockNumpadSource, setStockNumpadSource] = useState<"addDialog" | "editDialog" | null>(
    null,
  );
      ...
      <div className="pt-3">
        {loading ? (
          ...
          <div className="text-center py-20 text-muted-foreground">
            No{" "}
            {t(
              categoryKey(CATEGORIES.find((c) => c.value === category)?.value ?? ""),
            ...
            {filtered.map((p) => (
              ...
              >
                <div className="aspect-[3/4] relative w-full">
                  ...
                  <div className="absolute top-1.5 left-1.5 h-10 min-w-[2.5rem] px-2 rounded-full flex items-center justify-center bg-black/70 shadow z-10">
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        ...
                        <AlertDialogFooter className="flex-row gap-3 mt-2">
                          <AlertDialogCancel className="flex-1 h-14 text-base font-black m-0">
                            Cancel
                          </AlertDialogCancel>
                          <AlertDialogAction
                ...
                {(() => {
                  ...
                  return (
                    <div
                      className="px-1.5 py-1.5 pointer-events-none select-none"
                      style={{
                    ...
                    >
                      ...
                      <div
                        className="font-black text-[10px] leading-tight mt-0.5"
                        style={{ color: cpMissing ? "#f87171" : "var(--primary)" }}
                      >
                        CP: ${cp.toFixed(2)}
                      </div>
                      <div
                        className="font-black text-[10px] leading-tight"
                        style={{ color: spMissing ? "#f87171" : "var(--primary)" }}
                      >
                        SP: ${sp.toFixed(2)}
                      </div>
                    </div>
...
function AddItemDialog({
  ...
  onBulkSelect?: (templates: { url: string; label: string; category: string }[]) => void;
  ownerId: string;
  ...
  const [selectedTemplates, setSelectedTemplates] = useState<
    Map<string, { label: string; category: string }>
  >(new Map());
  ...
  useEffect(() => {
    if (activeNumpad && numpadRef.current) {
      setTimeout(
        () => numpadRef.current?.scrollIntoView({ behavior: "smooth", block: "nearest" }),
        50,
      );
    }
  }, [activeNumpad]);
  ...
  const submit = async () => {
              ...
              ? [
                  {
                    ...
                    label: "Drink",
                    units_consumed: 1,
                    price: parseFloat(shotPricePerUnit),
                  },
                ]
              : []),
          ...
          ? [
                ...
                ? [
                    {
                      ...
                      label: "Retail",
                      units_consumed: 1,
                      price: parseFloat(cigRetailPrice),
                    },
                  ]
                : []),
              ...(parseInt(cigSpecialQty) > 0 && parseFloat(cigSpecialPrice) > 0
                ? [
                    {
                      key: "special",
                      label: `${cigSpecialQty} for $${parseFloat(cigSpecialPrice).toFixed(2)}`,
                      units_consumed: parseInt(cigSpecialQty),
                      price: parseFloat(cigSpecialPrice),
                    },
                  ]
                : []),
            ]
    ...
    if (isEdit && editProduct) {
      ...
      if (!skipStockRef.current) {
        onSaved({
          ...updated,
          units_per_item: updated.units_per_item ?? 0,
          bottle_variations: (updated.bottle_variations ?? null) as any,
        });
      }
    } else {
      // ── INSERT new product ───────────────────────────────────────────────
      ...
      if (!skipStockRef.current) {
        onSaved({
          ...inserted,
          units_per_item: inserted.units_per_item ?? 0,
          bottle_variations: (inserted.bottle_variations ?? null) as any,
        });
      }
    }
  };
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-3">
          ...
          {showTemplates && onBulkSelect && (
            <button
              ...
              style={{
                background:
                  selectedTemplates.size > 0 ? "var(--gradient-hero)" : "rgba(255,255,255,0.08)",
              }}
            >
              Done{" "}
      ...
      <div
        ...
        style={{ scrollbarWidth: "thin", scrollbarColor: "rgba(251,146,60,0.4) transparent" }}
      >
        {showTemplates ? (
          <div className="flex flex-col h-full">
            {/* Sticky search + category tabs */}
            <div
              className="sticky top-0 z-10 pb-2 space-y-2"
              style={{ background: "var(--background)", paddingTop: "1px" }}
            >
              ...
              <div className="grid grid-cols-5 gap-2">
                {CATEGORIES.filter(
                  (cat) => cat.value !== "miscellaneous" && cat.value !== "food",
                ).map((cat) => (
                  <button
                    key={cat.value}
                    onClick={() => setTemplateCat(cat.value)}
                    className={`h-10 rounded-xl font-black transition flex items-center justify-center ${
                      templateCat === cat.value
                        ? "text-primary-foreground"
                        : "bg-muted text-muted-foreground hover:text-foreground"
                    }`}
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-3 items-stretch">
              <div
                className="relative w-1/2 aspect-[3/4] rounded-xl border-2 border-dashed border-border overflow-hidden shrink-0"
                style={{ background: "var(--gradient-card)" }}
              >
                {preview ? (
                  <img
            ...
            <p className="text-[10px] text-muted-foreground/70 leading-snug">
              💡 {t("best_results_tip", "For best results, upload a")}{" "}
              <span className="font-bold text-amber-400">
                {t("png_transparent", "PNG with transparent background")}
              </span>
              .
            </p>
            ...
            <div>
              ...
              <Input
                ...
                placeholder="Heineken 330ml"
                className="h-9"
              />
            </div>

            {/* Category + Add Qty + Total Cost */}
            <div className="space-y-0">
              <div className="flex gap-2">
                  ...
                  >
                    {CATEGORIES.map((cat) => (
                      <option key={cat.value} value={cat.value}>
                        {cat.icon} {t(categoryKey(cat.value), cat.label)}
                      </option>
                    ))}
                  </select>
                </div>
                <div className="flex-1">
                  ...
                  >
                    <span
                      className={`text-base font-black ${activeNumpad === "selling" ? "text-primary" : "text-muted-foreground"}`}
                    >
                      ${price || "0.00"}
                    </span>
                  </div>
              ...
              <div className="space-y-2">
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    >
                      <span
                        className={`text-base font-black ${activeNumpad === "units" ? "text-primary" : "text-muted-foreground"}`}
                      >
                  ...
                  <div>
                    ...
                    <div
                      className="mt-1 h-9 rounded-lg border border-border bg-muted/30 flex items-center px-3 cursor-pointer active:bg-muted/50 transition"
                      onClick={() =>
                        setActiveNumpad(activeNumpad === "shotprice" ? null : "shotprice")
                      }
                    >
                      <span
                        className={`text-base font-black ${activeNumpad === "shotprice" ? "text-primary" : "text-muted-foreground"}`}
                      >
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <div
                    className="mt-1 h-9 rounded-lg border border-border bg-muted/30 flex items-center px-3 cursor-pointer active:bg-muted/50 transition"
                    onClick={() =>
                      setActiveNumpad(activeNumpad === "cigretail" ? null : "cigretail")
                    }
                  >
                    <span
                      className={`text-base font-black ${activeNumpad === "cigretail" ? "text-primary" : "text-muted-foreground"}`}
                    >
                      ${cigRetailPrice || "0.00"}
                    </span>
                  </div>
                  ...
                  {cigRetailPrice &&
                    ...
                    parseInt(unitsPerItem) > 0 && (
                      <p className="text-xs mt-1" style={{ color: "var(--primary)" }}>
                        Full pack retail value: $
                        {(parseFloat(cigRetailPrice) * parseInt(unitsPerItem)).toFixed(2)}
                      </p>
                    )}
                </div>
                {/* Special offer */}
                ...
                >
                  ...
                  {cigSpecialQty &&
                    cigSpecialPrice &&
                    parseInt(cigSpecialQty) > 0 &&
                    parseFloat(cigSpecialPrice) > 0 && (
                      <p className="text-xs" style={{ color: "#86efac" }}>
                        {cigSpecialQty} for ${parseFloat(cigSpecialPrice).toFixed(2)} · $
                        {(parseFloat(cigSpecialPrice) / parseInt(cigSpecialQty)).toFixed(2)} each
                      </p>
                    )}
                </div>
              </div>
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        <input
                          ...
                          onChange={(e) =>
                            setBottleVariations((bv) =>
                              bv.map((x, j) => (j === i ? { ...x, label: e.target.value } : x)),
                            )
                          }
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          ...
                          <div
                            ...
                            onClick={() =>
                              setActiveNumpad(
                                activeNumpad === `var_${i}_shots` ? null : `var_${i}_shots`,
                              )
                            }
                          >
                            <span
                              className={`text-xs font-black ${activeNumpad === `var_${i}_shots` ? "text-primary" : "text-muted-foreground"}`}
                            >
                              {v.units_consumed || "0"}
                            </span>
                          </div>
                        ...
                        <div>
                          ...
                          <div
                            ...
                            onClick={() =>
                              setActiveNumpad(
                                activeNumpad === `var_${i}_price` ? null : `var_${i}_price`,
                              )
                            }
                          >
                            <span
                              className={`text-xs font-black ${activeNumpad === `var_${i}_price` ? "text-primary" : "text-muted-foreground"}`}
                            >
