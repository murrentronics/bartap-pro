...
function totalMinutesForDay(cards: TimeCardRow[]) {
  return cards.reduce((sum, tc) => {
    ...
    return (
      sum + Math.max(0, Math.round((out.getTime() - new Date(tc.clocked_in_at).getTime()) / 60000))
    );
  }, 0);
}
    ...
    >
      ...
      <div className="grid grid-cols-7 gap-y-1">
        ...
        {Array.from({ length: daysInMonth }).map((_, i) => {
          ...
          const isToday =
            dateStr === today.toLocaleDateString("en-CA", { timeZone: "America/Port_of_Spain" });
          return (
            <button
              key={day}
              disabled={!isWorked}
...
async function downloadOwnerTimesheetPdf(
  ...
  for (const { name, cards: empCards } of Object.values(byEmp)) {
    ...
    doc.setTextColor(...BRAND);
    doc.text(
      `Total: ${empTotalMins < 60 ? empTotalMins + "m" : totalH + "h " + totalM + "m"}`,
      RM,
      y + 3,
      { align: "right" },
    );
...
function HoursTab({ ownerId, barIsOpen }: { ownerId: string; barIsOpen: boolean }) {
  ...
  const [employees, setEmployees] = useState<
    { id: string; username: string; role: string; job_title?: string }[]
  >([]);
  ...
  const [tsStaffEmp, setTsStaffEmp] = useState<{
    id: string;
    username: string;
    role: string;
    job_title?: string;
  ...
  const loadEmployees = useCallback(async () => {
    ...
    setEmployees(
      (data ?? []) as { id: string; username: string; role: string; job_title?: string }[],
    );
  ...
  useEffect(() => {
    loadEmployees();
    loadCards();
  }, [loadEmployees, loadCards]);
  useEffect(() => {
    const ch = supabase
      .channel(`owner-timecards-${ownerId}`)
      .on(
        "postgres_changes",
  ...
  const handleClockIn = async () => {
    ...
    const { error } = await sb.from("time_cards").insert({
      ...
      work_date: tzNow().toLocaleDateString("en-CA"),
    });
    setClockBusy(false);
    if (error) {
  ...
  function getTsCards(): TimeCardRow[] {
    ...
    if (tsPeriod === "week") {
      ...
      s.setDate(ref.getDate() - dow);
      const e = new Date(ref);
      e.setDate(ref.getDate() + (6 - dow));
      return base.filter(
        (tc) =>
          tc.work_date >= s.toLocaleDateString("en-CA") &&
          tc.work_date <= e.toLocaleDateString("en-CA"),
      );
    }
    if (tsPeriod === "month")
      return base.filter((tc) => tc.work_date.startsWith(tsSelectedDate.slice(0, 7)));
    if (tsPeriod === "year")
      return base.filter((tc) => tc.work_date.startsWith(tsSelectedDate.slice(0, 4)));
    return base;
  }
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {isSel && empOpen && (
                        ...
                        >
                          Since {fmtClockTime(empOpen.clocked_in_at)} ·{" "}
                          {fmtWorkDuration(empOpen.clocked_in_at, null)} on shift
                        </p>
                      )}
                    </div>
                    {empOpen ? (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs mt-0.5" style={{ color: "rgba(134,239,172,0.8)" }}>
                        Since {fmtClockTime(tc.clocked_in_at)} ·{" "}
                        {fmtWorkDuration(tc.clocked_in_at, null)} on shift
                      </p>
                    </div>
                    <span
                      className="text-[10px] font-black px-2 py-0.5 rounded-full shrink-0"
          ...
          <div className="flex gap-2 items-center">
            ...
            <button
              onClick={async () => {
                ...
                setTsPdfBusy(true);
                try {
                  await downloadOwnerTimesheetPdf(
                    tsCards,
                              ...
                              {dOpen && (
                                <div className="divide-y divide-border/30 bg-black/10">
                                  {cards.map((tc) => {
                                    ...
                                    const outTime = tc.clocked_out_at
                                      ? fmtClockTime(tc.clocked_out_at)
                                      : null;
                                    const dur = fmtWorkDuration(
                                      tc.clocked_in_at,
                                      tc.clocked_out_at,
                                    );
                                      ...
                                      >
                                        <div
                                          ...
                                          style={{
                                            ...
                                            color: isAct ? "#86efac" : "var(--primary)",
                                          }}
                                        >
                                          {tc.employee_name.charAt(0).toUpperCase()}
                                        </div>
                    ...
                    {monthPayments.map((p) => {
                      ...
                      return (
                        <div key={p.id} className="flex items-center justify-between px-4 py-3">
                          ...
                          <p className="font-black text-sm" style={{ color: "#86efac" }}>
...
function SalaryTab({ cashiers, ownerId }: { cashiers: Cashier[]; ownerId: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      for (const row of due as (SalaryRecord & { profiles: { username: string } })[]) {
        ...
        const nextAt = row.frequency
          ? computeNextPayAt(row.frequency, row.pay_day ?? 1, row.pay_time ?? "18:00")
          : null;
        await supabase
          .from("cashier_salaries")
          .update({ last_paid_at: new Date().toISOString(), next_pay_at: nextAt })
          .eq("id", row.id);
      }
      if (due.length > 0) {
        toast.success(
          `${due.length} scheduled salary payment${due.length > 1 ? "s" : ""} auto-processed`,
        );
        loadSalaries();
      }
    })();
  ...
  const saveSalary = async (cashierId: string) => {
    ...
    const payload = {
      ...
      next_pay_at:
        formMode === "schedule"
          ? computeNextPayAt(formFreq, formPayDay, formTime, formIncludeThisWeek)
          : null,
      active: true,
    };
    let error;
  ...
  if (cashiers.length === 0)
    return <div className="text-muted-foreground py-10 text-center text-sm">No cashiers yet.</div>;
                    ...
                    {(() => {
                      ...
                      return (
                        <span
                          className="text-[10px] font-black px-1.5 py-0.5 rounded-full shrink-0"
                          style={{
                            background: "rgba(var(--primary-rgb,251 146 60)/0.15)",
                            border: "1px solid rgba(var(--primary-rgb,251 146 60)/0.4)",
                            color: "var(--primary)",
                          }}
                        >
                          Cashier
                  ...
                  {salary ? (
                    <>
                      <p className="text-xs text-muted-foreground">
                        ...
                        {!salary.frequency && (
                          <span className="text-muted-foreground"> · Pay Now only</span>
                        )}
                      </p>
                      {nextPayLabel(salary) && (
                        <p className="text-[10px] text-primary font-semibold">
                          Next: {nextPayLabel(salary)}
                        </p>
                      )}
                    </>
                  ) : (
                ...
                <div className="border-t border-border px-4 pb-4 pt-3 space-y-4">
                  ...
                  <div>
                    ...
                    {formMode === "now" && (
                      <p className="text-xs text-muted-foreground mt-2 text-center">
                        Enter amount and tap Pay Now to record instantly.
                      </p>
                    )}
                  </div>
                    ...
                    <>
                      ...
                      {formFreq !== "monthly" && (
                        <div>
                          ...
                          <button
                            type="button"
                            onClick={() => setFormIncludeThisWeek((v) => !v)}
                            className="mt-2 flex items-center gap-2 text-xs font-semibold text-muted-foreground hover:text-foreground transition"
                          >
                            <div
                              className="h-4 w-4 rounded border border-border flex items-center justify-center shrink-0"
                              style={
                                formIncludeThisWeek
                                  ? {
                                      background: "var(--gradient-hero)",
                        ...
                        <div>
                          ...
                          {formTime && (
                            <p
                              ...
                              style={{ color: "var(--primary)" }}
                            >
                              {new Date(`2000-01-01T${formTime}`).toLocaleTimeString("en-US", {
                                hour: "numeric",
                                minute: "2-digit",
                                hour12: true,
                              })}
                  ...
                  {confirmPayCashier?.id === c.id && (
                    <div
                      ...
                      onClick={() => setConfirmPayCashier(null)}
                    >
                      <div
                      ...
                      >
                        <div className="px-6 pt-6 pb-2 text-center">
                          ...
                          <p className="text-sm text-muted-foreground mt-1">
                            ...
                            to <span className="font-black text-foreground">{c.username}</span>?
                          </p>
                          <p className="text-xs text-muted-foreground mt-1">
                            This will be recorded as an expense immediately.
                          </p>
                        </div>
                  ...
                  {confirmScheduleCashier === c.id && (
                    <div
                      className="fixed inset-0 z-[80] flex items-center justify-center p-6 bg-black/70 backdrop-blur-sm"
                      onClick={() => setConfirmScheduleCashier(null)}
                    >
                      <div
                        className="w-full max-w-xs rounded-3xl border border-border shadow-2xl overflow-hidden"
                        style={{ background: "var(--gradient-card)" }}
                      ...
                      >
                        <div className="px-6 pt-6 pb-2 text-center">
                          ...
                          <p className="text-sm text-muted-foreground mt-1">
                            ...
                            for <span className="font-black text-foreground">{c.username}</span>?
                          </p>
                          <p className="text-xs text-muted-foreground mt-1">
                            {FREQ_LABELS[formFreq]}
                            {formFreq !== "monthly" && ` · ${DAYS_OF_WEEK[formPayDay]}`}
                            {formFreq === "monthly" &&
                              ` · ${formPayDay}${ordSuffix(formPayDay)} of month`}
                            {formFreq !== "monthly" &&
                              ` at ${new Date(`2000-01-01T${formTime}`).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit", hour12: true })}`}
                        ...
                        <div className="px-6 pb-6 pt-4 flex gap-3">
                          ...
                          <button
                            ...
                            disabled={saving}
                            onClick={() => {
                              setConfirmScheduleCashier(null);
                              saveSalary(c.id);
                            }}
...
function CashierStatement({
  ...
  const allRecords: CashierFlatRecord[] = [
    ...orders.map(
      (o): CashierFlatRecord => ({ kind: "order", data: o, ts: new Date(o.created_at).getTime() }),
    ),
    ...txs.map(
      (tx): CashierFlatRecord => ({ kind: "tx", data: tx, ts: new Date(tx.created_at).getTime() }),
    ),
  ].sort((a, b) => b.ts - a.ts);
  ...
  const handleDownload = async (month: string) => {
    ...
    try {
      ...
      const totalCreditPayments = txs
        .filter(
          (r) =>
            (r.data as WalletTx).type === "credit_payment" &&
            Number((r.data as WalletTx).amount) > 0,
        )
      ...
      const cols = [
        ...
        { label: "Credit Collected", value: "$" + totalCreditPayments.toFixed(2) },
        {
          ...
          value: "$" + (totalSales + totalCreditPayments - totalCleared).toFixed(2),
        },
      ];
      ...
      const filename =
        "cashier-statement-" + cashier.username + "-" + month.replace(/\s/g, "-") + ".pdf";
                          ...
                          if (rec.kind === "tx") {
                            ...
                            if (isCreditPayment) {
                              return (
                                <div
                                  key={tx.id}
                                  className="px-4 py-3 flex items-center gap-3 bg-blue-500/5"
                                >
                                  <div className="h-3.5 w-3.5 shrink-0 text-blue-400 font-black text-xs flex items-center justify-center">
                                    💳
                                  </div>
                                  <div className="flex-1 text-xs text-blue-300">
                                    {tx.note ?? "Credit payment"} ·{" "}
                                    {new Date(tx.created_at).toLocaleString("en-GB", {
                                      hour: "2-digit",
                                      minute: "2-digit",
                                      hour12: true,
                            ...
                            if (isCreditCharge) {
                              return (
                                <div
                                  key={tx.id}
                                  className="px-4 py-3 flex items-center gap-3 bg-amber-500/5"
                                >
                                  <div className="h-3.5 w-3.5 shrink-0 text-amber-400 font-black text-xs flex items-center justify-center">
                                    🪙
                                  </div>
                                  <div className="flex-1 text-xs text-amber-300">
                                    {tx.note ?? "Credit charge"} ·{" "}
                                    {new Date(tx.created_at).toLocaleString("en-GB", {
                                      hour: "2-digit",
                                      minute: "2-digit",
                                      hour12: true,
                          ...
                          return (
                            <div key={o.id} className="px-4 py-3">
                              ...
                              <div className="mt-0.5 text-xs text-muted-foreground">
                                Paid ${Number(o.paid).toFixed(2)} · Change $
                                {Number(o.change_given).toFixed(2)}
                              </div>
                            </div>
                          );
...
export default function CashiersPage() {
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on(
        ...
        { event: "UPDATE", schema: "public", table: "profiles", filter: `id=eq.${ownerIdForBar}` },
        (payload: any) => {
    ...
    const ch = supabase
      ...
      .on(
        "postgres_changes",
        {
          ...
          schema: "public",
          table: "profiles",
          filter: `parent_id=eq.${ownerIdForQuery}`,
        },
        () => load(),
      )
      .subscribe();
  ...
  const onCreate = async (e: React.FormEvent) => {
    ...
    if (/\s/.test(u)) {
      const m = "Username cannot contain spaces";
      setUsernameError(m);
      toast.error(m);
      return;
    }
    if (!/^[a-z0-9_]+$/.test(u)) {
      const m = "Lowercase letters, numbers and underscores only";
      setUsernameError(m);
      toast.error(m);
      return;
    }
  ...
  const onCreateCustom = async () => {
    if (!customName.trim()) {
      toast.error("Enter a name for this worker");
      return;
    }
    if (!customTitle.trim()) {
  ...
  const onClear = async (c: Cashier) => {
    ...
    load();
    refreshProfile();
    toast.success(`Balance cleared from ${c.username}`);
  };

  const onResetPassword = async () => {
    if (!resetPwCashier || newPw.length < 6) {
  ...
  const confirmOpenBarWithFloat = async () => {
    ...
    await (supabase as any)
      ...
      .insert({ owner_id: ownerIdForBar, amount: machineAmt, set_at: now });
          ...
          >
            <div className="px-6 pt-7 pb-4 text-center space-y-3">
              ...
              <p className="text-sm text-muted-foreground leading-snug">
                This will end the current session. Cashiers will not be able to make sales until the
                bar is reopened.
              </p>
            </div>
        ...
        <Tabs value={tab} onValueChange={setTab}>
          <TabsList className="grid grid-cols-4 w-full">
            ...
            <TabsTrigger value="manage">
              {t("tab_manage", "Manage")} ({list.length})
            </TabsTrigger>
            ...
            {!selectedRole && (
              <div className="mt-6 space-y-3">
                <p className="text-sm text-muted-foreground text-center">
                  {t("select_employee_type", "Select the type of employee to create")}
                </p>
                <div className="grid grid-cols-3 gap-3">
                  ...
                  >
                    <div
                      ...
                      style={{ background: "rgba(var(--primary-rgb,251 146 60)/0.15)" }}
                    >
                      💰
                    </div>
                    <span className="font-black text-sm">{t("role_cashier", "Cashier")}</span>
                    <span className="text-[10px] text-muted-foreground text-center leading-tight">
                      {t("cashier_desc", "Full bar access, requires login")}
                    </span>
                  </button>
                  {/* Manager */}
                  <button
                  ...
                  >
                    ...
                    <span className="text-[10px] text-muted-foreground text-center leading-tight">
                      {t("manager_desc", "Items, Wallet & Machines only")}
                    </span>
                  </button>
                  {/* Custom */}
                  <button
                  ...
                  >
                    ...
                    <span className="text-[10px] text-muted-foreground text-center leading-tight">
                      {t("custom_desc", "No login, salary tracking only")}
                    </span>
                  </button>
                </div>
              </div>
              ...
              >
                ...
                <div>
                  ...
                  {usernameError ? (
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Single word only. Lowercase letters, numbers or underscores.
                    </p>
                  )}
                </div>
                <div>
                ...
                >
                  {busy ? (
                    ...
                    <>
                      ...
                      {selectedRole === "manager" ? "Manager" : "Cashier"}
                    </>
                  )}
                </Button>
              </form>
              ...
              >
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder="e.g. Bouncer, DJ, Waitress"
                    autoComplete="off"
                  />
                </div>
                <Button
                  onClick={onCreateCustom}
                  disabled={busy || !customName.trim() || !customTitle.trim()}
                  className="w-full h-12 font-black"
                  style={{ background: "var(--gradient-hero)", color: "var(--primary-foreground)" }}
                >
                  {busy ? (
          ...
          <TabsContent value="manage">
            <div className="mt-6 space-y-2">
              {list.length === 0 && (
                <div className="text-muted-foreground py-8 text-center">No staff yet.</div>
              )}
              {list.map((c) => {
                const isCustom = (c as any).role === "custom";
                ...
                const roleBadge = isCustom
                  ? {
                      label: (c as any).job_title ?? "Worker",
                  ...
                  : isManager
                    ...
                    : {
                        label: "Cashier",
                        color: "rgba(var(--primary-rgb,251 146 60)/0.15)",
                        border: "rgba(var(--primary-rgb,251 146 60)/0.4)",
                        text: "var(--primary)",
                      };
                return (
                  <div
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <AlertDialog>
                        <AlertDialogTrigger asChild>
                          <Button size="sm" variant="destructive" className="h-9 w-9 p-0 shrink-0">
            ...
            >
              <div className="px-6 pt-6 pb-2 text-center">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  ...
                  <span className="font-bold text-foreground">{resetPwCashier.username}</span>
                </p>
              </div>
              <div className="px-6 pb-6 pt-4 space-y-4">
